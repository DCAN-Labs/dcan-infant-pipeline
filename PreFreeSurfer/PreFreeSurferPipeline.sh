...
if [ -n "${T1BrainMask}" ] ; then
    # The user has supplied a T1 brain mask. 
    
    #perform acpc alignment by aligning T1 to template and then T2 to T1_acpc
    acpc_align_script_T2=${HCPPIPEDIR_PreFS}/ACPCAlignment_with_crop.sh
    acpc_align_script_T1=${HCPPIPEDIR_PreFS}/ACPCAlignment_with_crop_T1.sh
    ...
    imcp ${T1BrainMask} ${T2wFolder}/${T2wImage}_acpc_brain_mask
    
    #${T1wFolder}/${T1wImage}_brain_mask

    # Use the supplied mask to extract the T1 brain using acpc-aligned T1.
    ...
    # Extract the T2 brain using the T1 mask that is already acpc-aligned.
    ${FSLDIR}/bin/fslmaths ${T2wFolder}/${T2wImage} -mas ${T2wFolder}/${T1wImage}_acpc_brain_mask ${T2wFolder}/${T2wImage}_brain
