...
${ANTSPATH}${ANTSPATH:+/}antsRegistration -d 3 \
        ...
        --metric MI["$ReferenceBrain","$T1wRestoreBrain",1,32] \
        ...
        --metric CC["$ReferenceBrain","$T1wRestoreBrain",1,4] \
        --convergence 5 \
        --shrink-factors 1 \
        --smoothing-sigmas 0 -v  # use fixed space mask only for nonlinear registration

# fake nonlinear warp -identity vector field.
${FSLDIR}/bin/fslmaths "$WD"/xfms/antsreg1Warp.nii.gz -mul 0 "$WD"/xfms/antsreg1Warp.nii.gz
${FSLDIR}/bin/fslmaths "$WD"/xfms/antsreg1InverseWarp.nii.gz -mul 0 "$WD"/xfms/antsreg1InverseWarp.nii.gz
...
${FSLDIR}/bin/applywarp --rel --interp=spline -i ${T1wImage} -r ${Reference} -w ${OutputTransform} -o "$WD"/antswarpedT1w.nii.gz

# fake so we're undoing everything...
#fslmaths ${OutputTransform} -mul 0 ${OutputTransform}
#fslmaths ${OutputInvTransform} -mul 0 ${OutputInvTransform}
#fslmaths "$WD"/xfms/NonlinearRegJacobians.nii.gz -bin "$WD"/xfms/NonlinearRegJacobians.nii.gz
