...
# DCAN - epi reg is poor for babies because of contrast differences
${FSLDIR}/bin/flirt -in ${WD}/${T2wImageBasename} -ref ${WD}/${T1wImageBasename} -cost mutualinfo -searchrx -15 15 -searchry -15 15 -searchrz -15 15 -dof 6 -omat ${WD}/T2w2T1w/T2w_reg.mat -out ${WD}/T2w2T1w/T2w_reg
#${ANTSPATH}${ANTSPATH:+/}antsRegistration -d 3 \
#	--output "$WD"/antsreg \
#	--transform Rigid["0.1"] \
#	--metric MI["${WD}/${T1wImageBasename}","${WD}/${T2wImageBasename}",1,32] \
#	--convergence 1000x500x250x100 \
#	--shrink-factors 6x4x2x1 \
#	--smoothing-sigmas 3x2x1x0

#${C3DPATH}${C3DPATH:+/}c3d_affine_tool -itk antsreg0GenericAffine.mat -ref "${WD}/${T1wImageBasename}" -src "${WD}/${T2wImageBasename}" -ras2fsl -o ${WD}/T2w2T1w/T2w_reg.mat
...
# create T2w brain mask
${FSLDIR}/bin/applywarp --rel --interp=nn --in=${T2wImageBrain} --ref=${T1wImage} --warp=${WD}/T2w2T1w/T2w_dc_reg --out=${OutputT2wImage}_brain
${FSLDIR}/bin/fslmaths ${OutputT2wImage} -mas ${OutputT2wImage}_brain ${OutputT2wImage}_brain
fi
