...
echo " "
echo " START: BrainExtraction_FNIRT_mod"
...
# get output images from prep stage (may be relative to FNL structure):
if [ $Input = *"T1w_acpc"* ]; then
  PipelineFolder=$( dirname $( dirname $WD ) )
  flirt -in ${PipelineFolder}/masks/*_T1w_MPR_average_brain.nii.gz -r ${Reference} -applywarp -init ${WD}/../xfms/acpc.mat ${OutputBrainExtractedImage}
  flirt -interp nearestneighbour -in ${PipelineFolder}/masks/aseg.nii.gz -r ${OutputBrainExtractedImage} -applywarp -init ${WD}/../xfms/acpc.mat ${WD}/../aseg_acpc.nii.gz
  flirt -interp nearestneighbour -in ${WD}/../aseg_acpc.nii.gz -ref ${WD}/../aseg_acpc.nii.gz -applyisoxfm 1 -out ${WD}/../aseg_acpc_1mm.nii.gz
  ${FSLDIR}/bin/fslmaths ${OutputBrainExtractedImage} -bin $OutputBrainMask
elif [ $Input = *"T2w_acpc"* ]; then
  PipelineFolder=$( dirname $( dirname $WD ) )
  flirt -in ${PipelineFolder}/masks/*_T2w_SPC_average_brain.nii.gz -r ${Reference} -applywarp -init ${WD}/../xfms/acpc.mat ${OutputBrainExtractedImage}
  ${FSLDIR}/bin/fslmaths ${OutputBrainExtractedImage} -bin $OutputBrainMask
fi


echo " "
echo " END: BrainExtraction_FNIRT_mod"
