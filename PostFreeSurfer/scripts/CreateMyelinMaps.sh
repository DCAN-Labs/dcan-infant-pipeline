...
# Bias field mapping may or may not actually help. If there is no bias-field
# algorithm yet found that helps infants, the ${BiasField} file will not exist
# when we come into this script. In that case, skip the steps that use it - the
# output files will be the same as input for those steps.
if [ -e ${BiasField} ] ; then
    fslmaths ${OutputT1wImage} -div ${BiasField} ${OutputT1wImageRestore}
else
    imcp ${OutputT1wImage} ${OutputT1wImageRestore}
fi
...
if [ -e ${BiasField} ] ; then
    ...
    imcp ${OutputMNIT1wImage} ${OutputMNIT1wImageRestore}
fi
...
if $useT2; then
    ...
    if [ -e ${BiasField} ] ; then
        ...
        imcp ${OutputT2wImage} ${OutputT2wImageRestore}
    fi
    ...
    if [ -e ${BiasField} ] ; then
        ...
        imcp ${OutputMNIT2wImage} ${OutputMNIT2wImageRestore}
    fi
