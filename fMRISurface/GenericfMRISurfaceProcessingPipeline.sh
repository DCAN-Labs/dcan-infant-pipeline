...
SubcorticalMapMethod=`opts_GetOpt1 "--subcorticalmapmethod" $@`
RepetitionTime=`opts_GetOpt1 "--repetitiontime" $@`
...
log_Msg "SubcorticalMapMethod: ${SubcorticalMapMethod}"
log_Msg "RepetitionTime: ${RepetitionTime}"
log_Msg "RUN: ${RUN}"
...
if [ -n "${SubcorticalMapMethod}" ] && [[ "MNI_AFFINE" == "${SubcorticalMapMethod}" ]] ; then
    # Old, not very good method, aligning the whole subcortical area as a single blob.
    "$PipelineScripts"/SubcorticalAlign_MNI_Affine.sh "$AtlasSpaceFolder" "$ROIFolder" "$FinalfMRIResolution" "$ResultsFolder" "$NameOffMRI" "$SmoothingFWHM" "$GrayordinatesResolution"
else
    # Default is new method: align each ROI separately and then combine (aka ROI_MAP).
    "$PipelineScripts"/SubcorticalAlign_ROIs.sh ${ROIFolder} ${ResultsFolder} ${NameOffMRI} ${SmoothingFWHM} ${GrayordinatesResolution} ${RepetitionTime}
fi
