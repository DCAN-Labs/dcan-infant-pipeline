...
MotionCorrectionFrame=`opts_GetOpt1 "--mcframe" $@`  # Frame from which to make Scout file.
...
T1wFolder="T1w" #Location of T2w images
T2wFolder="T2w"
T2wPrepImage="T2w"
...
sebasedBiasFieldMNI="$Path/$AtlasSpaceFolder/Results/$NameOffMRI/${NameOffMRI}_sebased_bias.nii.gz"
...
if [ -n "${MotionCorrectionFrame}" ] ; then
    # Frames (aka volumes) are numbered starting at 0, so subtract 1.
    MotionCorrectionFrame=$((MotionCorrectionFrame-1))
else
...
if [ $fMRIScout = "NONE" ] ; then
  ...
  echo Copying $fMRIScout to $OrigScoutName.
...
# hotfix for infants - simple registration until better refinement is decided.
...
then
    ...
    then
        ${FSLDIR}/bin/applywarp --interp=trilinear -i ${fMRIFolder}/DistortionCorrectionAndEPIToT1wReg_FLIRTBBRAndFreeSurferBBRbased/ComputeSpinEchoBiasField/sebased_bias_dil.nii.gz -r ${fMRIFolder}/${NameOffMRI}_SBRef_nonlin -w ${Path}/MNINonLinear/xfms/acpc_dc2standard.nii.gz -o ${Path}/MNINonLinear/Results/${NameOffMRI}/${NameOffMRI}_sebased_bias.nii.gz
        ${FSLDIR}/bin/fslmaths ${Path}/MNINonLinear/Results/${NameOffMRI}/${NameOffMRI}_sebased_bias.nii.gz -mas ${fMRIFolder}/${FreeSurferBrainMask}.${FinalfMRIResolution}.nii.gz ${Path}/MNINonLinear/Results/${NameOffMRI}/${NameOffMRI}_sebased_bias.nii.gz

        ${FSLDIR}/bin/applywarp --interp=trilinear -i ${fMRIFolder}/DistortionCorrectionAndEPIToT1wReg_FLIRTBBRAndFreeSurferBBRbased/ComputeSpinEchoBiasField/sebased_reference_dil.nii.gz -r ${fMRIFolder}/${NameOffMRI}_SBRef_nonlin -w ${Path}/MNINonLinear/xfms/acpc_dc2standard.nii.gz -o ${Path}/MNINonLinear/Results/${NameOffMRI}/${NameOffMRI}_sebased_reference.nii.gz
        ${FSLDIR}/bin/fslmaths ${Path}/MNINonLinear/Results/${NameOffMRI}/${NameOffMRI}_sebased_reference.nii.gz -mas ${fMRIFolder}/${FreeSurferBrainMask}.${FinalfMRIResolution}.nii.gz ${Path}/MNINonLinear/Results/${NameOffMRI}/${NameOffMRI}_sebased_reference.nii.gz

        ${FSLDIR}/bin/applywarp --interp=trilinear -i ${fMRIFolder}/DistortionCorrectionAndEPIToT1wReg_FLIRTBBRAndFreeSurferBBRbased/ComputeSpinEchoBiasField/${NameOffMRI}_dropouts.nii.gz -r ${fMRIFolder}/${NameOffMRI}_SBRef_nonlin -w ${Path}/MNINonLinear/xfms/acpc_dc2standard.nii.gz -o ${Path}/MNINonLinear/Results/${NameOffMRI}/${NameOffMRI}_dropouts.nii.gz
    fi
fi
