...
T1wImage=`getopt1 "--t1" $@`
T2wPrepImage=`getopt1 "--t2" $@`
T1wRestoreImage=`getopt1 "--t1restore" $@`
T2wRestoreImage=`getopt1 "--t2restore" $@`
T1wBrainImage=`getopt1 "--t1brain" $@`
T2wBrainImage=`getopt1 "--t2brain" $@`
...
case $DistortionCorrection in
    ...
    ${FIELDMAP_METHOD_OPT} | ${SIEMENS_METHOD_OPT} | ${GENERAL_ELECTRIC_METHOD_OPT}) #@WHY is this ) HERE
	...
	#EMMA DOES magnitude_brain come from? no FM no mag?
    ...
    ${SPIN_ECHO_METHOD_OPT})
        ...
        # NOT USING T1w FOR EPI REG IN BABIES
        if false; then
        ...
        ${FSLDIR}/bin/applywarp --rel --interp=spline -i ${ScoutInputName} -r ${T1wImage} -w ${WD}/${ScoutInputFile}_undistorted2T1w_init_warp -o ${WD}/${ScoutInputFile}_undistorted2T1w_init
          ...
          echo "Neonatal Pipeline:  Using T2w image for epi registration"
          log_Msg "register undistorted scout image to T2w, then combine with T2w to T1w"
          # may refine this heavily in a future iteration, for now, simple flirt gives excellent results
          flirt -in ${WD}/${ScoutInputFile}_undistorted -dof 6 -cost mutualinfo -ref $T2wRestoreImage -omat ${WD}/${ScoutInputFile}_undistorted2T2w_init.mat -out ${WD}/${ScoutInputFile}_undistorted2T2w_init.nii.gz
          cp ${WD}/${ScoutInputFile}_undistorted2T2w_init.mat  ${WD}/${ScoutInputFile}_undistorted2T1w_init.mat
          flirt -in ${WD}/${ScoutInputFile}_undistorted -ref $T1wRestoreImage -applyxfm -init ${WD}/${ScoutInputFile}_undistorted2T1w_init.mat -out ${WD}/${ScoutInputFile}_undistorted2T1w_init.nii.gz
          ...
          cp "${WD}/${ScoutInputFile}_undistorted2T1w_init.mat" "${WD}/fMRI2str.mat"
          ${FSLDIR}/bin/convertwarp --relout --rel --warp1=${WD}/${ScoutInputFile}_undistorted2T1w_init_warp.nii.gz --ref=${T1wImage} --out=${WD}/fMRI2str.nii.gz #  --postmat=${WD}/fMRI2str.mat this shouldn't have been here.
        fi
    ...
    ${T2_METHOD_OPT})
        ...
        # --------------------------
        # -- No Spin Echoes, No FM, ANTS T2 distortion correction --
        # --------------------------

        # Use topup to distortion correct the scout scans
        # using a blip-reversed SE pair "fieldmap" sequence
        ${GlobalScripts}/T2DistortionCorrection.sh \
            --workingdir=${WD}/FieldMap \
            --scoutin=${ScoutInputName} \
	    	--t2in=${T2wPrepImage} \
            --owarp=${WD}/WarpField \
    ...
    NONE)
		...
		#ABCD LOGIC
        #${FSLDIR}/bin/flirt -in ${ScoutInputName}.nii.gz -dof 6 -cost mutualinfo -ref $T1wRestoreImage -omat ${WD}/fMRI2str.mat -out ${WD}/${ScoutInputFile}_undistorted2T1w_init.nii.gz
		#cp ${WD}/${ScoutInputFile}_undistorted2T2w_init.nii.gz ${WD}/${ScoutInputFile}_undistorted2T1w_init.nii.gz #testing different T1
		#2T1w_init even tho 2T2
...
if [[ $UseJacobian == "true" ]] ; then
    log_Msg "apply Jacobian correction to scout image"
    if [[ -n "${UseBiasField}" ]] && [ -e ${UseBiasField} ]
    then
...
# T2w refinement already performed
if true; then
	if [ -e ${FreeSurferSubjectFolder}/${FreeSurferSubjectID}_1mm ] ; then
	  ...
	  log_Msg "Not running BBR for neonates"
...
#${FSLDIR}/bin/convert_xfm -omat ${WD}/fMRI2str.mat -concat ${WD}/fMRI2str_refinement.mat ${WD}/${ScoutInputFile}_undistorted2T1w_init.mat
  ...
  log_Msg "useT2=false  Using original ${WD}/${ScoutInputFile}_undistorted2T1w_init.mat, no refinement done AP 20162111"
  log_Msg "${FreeSurferSubjectFolder}/${FreeSurferSubjectID}_1mm does not exist. FreeSurferNHP.sh was not used."
  ...
  log_Msg "Create FSL-style matrix and then combine with existing warp fields"
  ${FREESURFER_HOME}/bin/tkregister2 --noedit --reg ${WD}/EPItoT1w.dat --mov ${WD}/${ScoutInputFile}_undistorted2T1w_init.nii.gz --targ ${T1wImage}.nii.gz --fslregout ${WD}/fMRI2str_refinement.mat
    #If DistrotionCorrection = "NONE" then there is no warping being done so it's not necessary to create fMRI2str.nii.gz, which I believe is just a warp field used to refine ${ScoutInputFile}_undistorted2T1w_init.mat. Skip this and just use the orignally created undistorted2T1w_init. - Anders Perrone 20161212 #EMMA ADDED TO INFANT 3/12
	if [[ $DistortionCorrection == "NONE" ]]; then
    	${FSLDIR}/bin/convertwarp --relout --rel -m ${WD}/fMRI2str.mat --ref=${T1wImage} --out=${WD}/fMRI2str.nii.gz
	else
  ${FSLDIR}/bin/convertwarp --relout --rel --warp1=${WD}/${ScoutInputFile}_undistorted2T1w_init_warp.nii.gz --ref=${T1wImage} --postmat=${WD}/fMRI2str_refinement.mat --out=${WD}/fMRI2str.nii.gz
